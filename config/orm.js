...
const url = 'mongodb://localhost:27017/flashmob_db';
...
function createRecord(obj, cnName, cb) {
  ...
  MongoClient.connect(url, (err, client) => {
    ...
    collection.insertOne(obj, (error, res) => {
      if (error) {
...
function getQuery(query, cnName, cb) {
  MongoClient.connect(url, (err, client) => {
    ...
    collection.find(query).toArray((error, res) => {
      if (error) {
...
function updateEntry(query, updateData, cnName, cb) {
  ...
  MongoClient.connect(url, (err, client) => {
    ...
    const db = client.db(dbName);
    const collection = db.collection(cnName);
    collection.updateOne(query, updateData, (error, res) => {
...
function deleteEntry(myquery, cnName, cb) {
  MongoClient.connect(url, (err, client) => {
    ...
    const db = client.db(dbName);
    const collection = db.collection(cnName);
    collection.deleteOne(myquery, (error, res) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      lookForExisting(query, 'USERS', (err, res) => {
        if (res) {
          ...
          createRecord(myobj, 'USERS', (error, result) => {
            if (error) throw error;
            resolve(result);
          });
        }
